...
public:
    Calibration()
        ...
        // Configuración de la cámara (asumiendo que ya tienes tus parámetros)
...
private:
    void listener_callback_color_image(const sensor_msgs::msg::Image::SharedPtr msg) {
        ...
        // Detectar marcadores ArUco y calcular transformaciones
        std::vector<int> ids;
        ...
        if (!ids.empty()) {
            ...
            // Dibujar marcadores y calcular transformaciones
            ...
            // Mostrar imagen (descomentar para habilitar la interacción con la ventana)
            cv::imshow("out", im_color);
            (char)cv::waitKey(100);

            // Publicar transformación del primer marcador ArUco
            if (!rvecs.empty()) {
    ...
    void publish_aruco_tf(const cv::Mat& rotation_vector_aruco, const cv::Mat& translation_vector_aruco, int id) {
        ...
        T.col(3).head(3) << translation_vector_aruco.at<double>(0), translation_vector_aruco.at<double>(1), translation_vector_aruco.at<double>(2);
        T.row(3) << 0, 0, 0, 1;
        ...
        // Supongamos que tienes un punto (1.0, 2.0, 3.0) en el sistema de referencia de la cámara
